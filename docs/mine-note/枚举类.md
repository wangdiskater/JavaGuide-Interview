...
首先，对于编译时安全性，我们看另一个示例，两个不同枚举类型进行比较：
...
![image-20210507100747891](https://i.loli.net/2021/05/07/CuDvXz3Zp7Jd8VL.png)

那是因为enum的equal方法第二个参数是Object类型，所以没报错
...
让我们看一下以下代码段中的运行时安全性，其中 `==` 运算符用于比较状态，并且如果两个值均为null 都不会引发 NullPointerException。相反，如果使用equals方法，将抛出 NullPointerException：
